...
(async () => {
  ...
  page.on('request', request => {
    if (request.url().includes('127.0.0.1:8787')) {
      console.log(`[NETWORK] ${request.method()} ${request.url()} | Origin: ${request.headers().origin}`);
    }
    request.continue();
  });
  page.on('response', response => {
    if (response.url().includes('127.0.0.1:8787')) {
      console.log(`[NETWORK] Response ${response.status()} | ACAO: ${response.headers()['access-control-allow-origin'] || 'none'}`);
    }
  });
  ...
  console.log(`[PASS] GET Status Rendered: ${statusHtml.includes('likelihood') || statusHtml.includes('Health')}`);
  ...
  await page.waitForFunction('document.getElementById("sub-result").textContent.includes("Validation")');
  ...
  await page.click('#subscribe-btn');
  await page.waitForFunction('document.getElementById("sub-result").textContent.includes("Failed to fetch")');
