...
export class OrchestrationLockRepository {
  ...
  async acquire(
    ...
    try {
      const existing = await this.db
        .prepare(`SELECT * FROM orchestration_locks WHERE name = ?1`)
        .bind(name)
        .first<OrchestrationLockRow>();
      ...
      if (!existing) {
        ...
        const res = await this.db
          ...
          .bind(name, ownerRunId, nowIso, expiresAtIso)
          .run();
      ...
      if (existing.expires_at <= nowIso) {
        ...
        const res = await this.db
          ...
          .bind(ownerRunId, nowIso, expiresAtIso, name, previousOwnerId)
          .run();
      ...
      return {
        ...
        activeOwnerId: existing.owner_run_id,
  ...
  async release(name: string, ownerRunId: string): Promise<boolean> {
    const res = await this.db
      .prepare(`DELETE FROM orchestration_locks WHERE name = ?1 AND owner_run_id = ?2`)
